...
CRAFT_LIST_STEP = 26  # row pitch; ten crafts fit above the stats at this and CRAFT_LIST_ICON
...
CRAFTS_ROW_TOP = 470  # first stat row baseline, clear of the ten craft rows. Was 430, which the
# bitcoin farm's row (the tenth, whose icon reaches down to y 446) reached down into: its Total
# rule sat at 417. 460 would clear it by a single pixel, so this is 470, putting the rule at 457
# and leaving 11px of gap. The panel runs to y 534 and the RUN TIME line lands at 496, so the
# band below had the room. An eleventh craft needs a tighter CRAFT_LIST_STEP, not more of this.
...
CRAFTS = (('slickers', 'slickers.png', (('CRACKERS', 'crackers', 'crackers.png'),
          ...
          # The bitcoin farm buys nothing, so it is the one row with no inputs: an empty tuple
          # draws no icons and leaves its settings view holding only SAVE. The enable box is the
          # whole point of the row being here, since build() runs any craft without one.
          ('bitcoin', 'bitcoin.png', ()))
...
class App:
    ...
    def _gold_ruble(self):
        """A gold-tinted rouble glyph PhotoImage and its width. The source png is a black glyph on
        transparency, so tinting keeps the alpha and repaints the colour; aspect is preserved."""
        from PIL import Image, ImageTk
        base = Image.open(CRAFT_ICONS_DIR / RUBLE_ICON).convert('RGBA')
        width = max(1, round(base.width / base.height * RUBLE_ICON_H))
        gold = Image.new('RGBA', base.size, (*RUBLE_GOLD, 0))
        gold.putalpha(base.getchannel('A'))
    ...
    # The crafts tab's <stem>_max fields are typed inputs that save themselves through a
    # StringVar trace (see _number_input); the <stem>_source pickers save through _save_pref.
    ...
    def start(self):
        ...
        except (window.WindowError, ValueError) as e:
            # WindowError: the game is on the wrong screen. ValueError: a mode refused to build,
            # eg crafts with no craft ticked. Either way, say why and do not start the countdown.
            self.bot = None
