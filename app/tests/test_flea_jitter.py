...
everything back on the way out. No game needed: time.sleep is stubbed to a recorder, the clicks
to no-ops, and random.uniform is pinned, so nothing sleeps or moves the mouse.

The load-bearing branch is the caller gate: a time.sleep from the sell code is padded, the same
call from inside pyautogui (its PAUSE and its tweened drags) is not, or a 0.4s drag would balloon
by up to a second per tween step. The two exec'd stubs carry the module names the gate reads.
"""
import os
import random
import sys
