"""Where the tick is looked for, and whether it is found there.

The autoselect similar button is matched, its right half taken (the checkbox sits at that end
of every reference crop of it), that half grown by CHECKMARK_MARGIN per side, and the checkmark
searched for inside. This is the geometry `sell.autoselect_similar_region` produces, checked
against whole frames rather than a live screen.

Every frame is run twice, once at its own size and once resized to 1920x1080, because the
references are 1080p crops and everything about this only goes wrong on a screen that is not.
The 1080p pass is a stand-in, not a real 1080p screen: the frame recorder writes what the
monitor is, and there are no 1080p frames to hand. Point this at one when there is.

Writes an annotated png per frame per size into tests/output/checkmark_region/: the matched
button in blue, the searched region in yellow, the checkmark in green when one is found. Look
at those before believing a 'no tick here' line, since a region drawn in the wrong place and a
box that genuinely has no tick in it read identically in the summary.

    python tests/test_checkmark_region.py                  # every frame the recorder has
    python tests/test_checkmark_region.py some/frame.png    # or the ones named
"""
import os
import sys
...
OUTPUT = Path(__file__).parent / 'output' / 'checkmark_region'
