...
App layer under test: interact/sell.py, sell.infer_scav_case_region, along with the finds it
leans on (find.find(sell.SCAV_WINDOW_TARGET) for the title bar and find.find_all(
sell.CLOSE_BUTTON_TARGET) for the close buttons). Verifies the inferred (left, top, w, h) box
falls where the open case window sits by drawing every piece onto a screenshot; raises
LookupError if a piece is missing.

Needs the live game: it screenshots the running window. Read-only, it never clicks. The scav
case window must already be open.

Run:  python tests/flea_scav/test_scav_case_region.py

Writes tests/output/scav_case_region.png: the title bar and close buttons boxed in lime,
the close button actually used in cyan, the inferred region in magenta.
"""
import sys
