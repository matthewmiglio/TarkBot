"""Does SCAV CASES ONLY refuse the stash when there is no case to open.

App layer under test: sell_bot.py, FleaSeller.select_item and its SCAV-CASES-ONLY fallback,
driving a fake interact/sell.py so open_scav_case / select_item_from_random_scav_case /
select_item_from_inventory can be scripted. Verifies that with no case on screen scav-only
returns an empty Selection and never touches the stash, while the other modes keep the
inventory fallback they always had (non-zero exit on any wrong branch).

No-game stub test: FleaSeller is built without __init__ and interact/sell.py is replaced by a
fake, so nothing reads a screen or wants a window.

Run:  python tests/flea_scav/test_scav_only_fallback.py
