...
Needs the live game: it moves the mouse and really clicks (unless --dry). Tarkov must be the
visible window.

Run:  python tests/flea_scav/test_open_scav_case.py        (countdown, then really clicks)
      python tests/flea_scav/test_open_scav_case.py --dry  (show the case it would pick, no clicking)

Writes tests/output/open_scav_case_before.png (every case boxed, the pick in magenta)
and _after.png. Exits non-zero if nothing was opened.
"""
import random
import sys
import time
