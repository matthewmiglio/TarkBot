"""Does window.state() tell the control panel and the game apart across the three overlap cases.

WHAT LAYER THIS TESTS
    window.py's overlap logic: window.state(gui_rect, game_rect, focus) and its CLEAR /
    GUI_ON_TOP / GAME_ON_TOP verdicts, the pure decision behind overlap_state(). This is the
    OS/window layer that tells whether our translucent control panel is sitting in front of the
    fullscreen game (a hole in anything read off it) or behind it (invisible but harmless).

WHY IT EXISTS
    Only state() is worth a test: the win32 lookups either find a window or raise, so the risk is
    all in the geometry and the focus rule. The made-up rectangles below (a 1920x1080 game at the
    origin with a 420x600 panel placed around it) cover the three the caller asked for plus the
    edges that break naive maths: touching edges, a one-pixel overlap, a minimised panel parked at
    Windows' -32000 corner, and a monitor left of the primary where coords go negative.

WHAT IT DOES  (NO GAME AND NO GUI NEEDED: pure rectangles)
    Feeds each case to window.state() and checks the verdict; collects failures and exits non-zero
    if any disagree. --live additionally reads the two real windows and prints their overlap_state.

Run:  python tests/platform/test_window_overlap.py          the decision, against made up rectangles
      python tests/platform/test_window_overlap.py --live   print what the real two windows are doing
"""
import sys
