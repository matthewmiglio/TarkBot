"""Booting and closing the game: the four launcher steps, what each refuses to do, which card it clicks.

WHAT LAYER THIS TESTS
    game_client/tarkov.py's process/launcher layer: start_tarkov (the four-wait boot: launch the
    BSG launcher, click Play, wait for the game region, click a profile card, confirm the lobby),
    close_game (forced taskkill), and the _profile_buttons clustering. This is the OS/game-client
    layer beneath the screen-reading bot; it drives no interact/ code and matches no pixels.

WHY IT EXISTS
    start_tarkov is four waits because none of the earlier steps work alone, measured on
    2026-08-30: EscapeFromTarkov.exe runs but draws no window without a session token,
    BsgLauncher.exe opens then sits, and the profile screen waits for a card. Two things carry the
    risk: which card is clicked is decided by column alone (the three SELECT buttons are
    pixel-identical, so a bug picks the wrong profile in silence), and every step that cannot find
    its target must click nothing (a click at a never-found button lands wherever the cursor was,
    on a screen showing the game).

WHAT IT DOES  (NO GAME NEEDED: the launcher, mouse, matcher and clock are all stubbed)
    Asserts each profile boots and clicks at its own column, a running game is left alone, each of
    the three missing-target cases clicks nothing past what it found, the six raw matches collapse
    to three cards by centre, a half-drawn screen reads as not-ready, and a live game is force-killed
    while a gone one is not. Prints an ok line on success, raises AssertionError otherwise.
...
sys.path.insert(0, str(Path(__file__).resolve().parents[2] / 'game_client'))
