"""Set the flea market filters to roubles, from players only.
...
Drives your mouse: opens the filter window, ticks remember-filters, works both dropdowns,
then OKs out. Anything already set is left alone, so running it twice in a row is safe and
the second run should touch nothing but the OK button.

Writes tests/output/flea_filters_before.png and _after.png (full window). Exits non-zero if
a step failed or the filters did not end up set.
"""
import sys
import time
...
def report(region):
    ...
    for label, target in (('filter button', sell.FILTER_BUTTON_TARGET),
                          ...
                          ('remember: on', sell.REMEMBER_ON_TARGET),
                          ('remember: off', sell.REMEMBER_OFF_TARGET),
