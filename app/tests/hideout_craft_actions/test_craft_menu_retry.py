...
Run:  python tests/hideout_craft_actions/test_craft_menu_retry.py

App layer: interact/craft.py's _open_item_menu (the right-click 'filter by item' menu action,
its hover-then-press and step-off-the-slot retry) and craft_bot.HideoutCraft.step, which turns a
menu that never opens into a craft swap rather than a raise. A craft-actions test: it pins the
right-click behaviour and the swap, not a plain read.
