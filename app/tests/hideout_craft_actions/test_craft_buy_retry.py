...
Run:  python tests/hideout_craft_actions/test_craft_buy_retry.py

App layer: interact/craft.py's buy_craft_input_item, the per-ingredient flea-buy action, its
refresh-in-place retry (capped at BUY_ATTEMPTS) and the four distinct Unbuyable messages (over
ceiling, outbid, unreadable, no PURCHASE button) the runner uses to leave the ingredient and
swap. A craft-actions test with the mouse, clock, matcher and flea all stubbed, so it needs no
game. The mouse, the clock, the matcher, the flea and the escape back to the station
