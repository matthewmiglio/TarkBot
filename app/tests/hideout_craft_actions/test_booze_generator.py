...
Run:  python tests/hideout_craft_actions/test_booze_generator.py --dry     read the row, click nothing
      python tests/hideout_craft_actions/test_booze_generator.py           run one pass for real
      python tests/hideout_craft_actions/test_booze_generator.py --purified-water-max 140000 --sugar-max 48900

App layer: craft_bot.HideoutCraft.step, the ordinary craft state machine (over
interact/craft.py's read_craft), driving the moonshine craft: collect, START and confirm the
handover, or buy the missing input off the flea. A craft-actions test that clicks START and can
spend real roubles on inputs: it needs the live game.
