...
Run:  python tests/hideout_craft_actions/test_craft_station_grouping.py

App layer: craft_bot.build (which orders the job cycle by station) and
craft_bot.HideoutCraft._swap (which navigates only when the next job is at a different station).
A craft-actions test: it pins the swap's navigate-or-not decision, not a plain read.
