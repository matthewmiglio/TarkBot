...
Exercises interact/sell.py's orientate_offer_creation and its _corner_point helper. The window
is dragged to a fixed corner so the inventory below it is always framed the same way for
find_sell_pixels; a bad corner name is refused up front by _corner_point with the valid list,
rather than halfway through a drag. Writes tests/flea_sell/output/orientate_offer_before.png and
_after.png, and re-finds the window afterwards to report how far it actually travelled, since
orientate_offer_creation returns the point it grabbed whether or not the game honoured the drag.

This one drags for real (unless --dry, which locates only). Needs the live game with the offer
creation window open and Tarkov visible. Exits non-zero if the window was not found.

Run:  python tests/flea_sell/test_orientate_offer.py                       (bottom left, the default)
      python tests/flea_sell/test_orientate_offer.py "bottom right"        (or any corner sell knows)
      python tests/flea_sell/test_orientate_offer.py "bottom right" --dry  (locate it only, no dragging)
"""
import sys
import time
