...
Stress-tests interact/sell.py's infer_inventory_region: each round drags the offer creation
window to a random point, then looks for all three framing buttons (sell.EDGES) that
infer_inventory_region measures the stash grid from. The first round where one stops matching
prints which, writes the screen to tests/flea_sell/output/inventory_region_stress.png, and exits
1. It is hunting for a window position where a framing button falls off screen or off-match, so
the region read cannot be trusted at every layout.

This one drags for real. Needs the live game, flea open with the offer creation window on
screen, since that window is the thing being dragged.

Run:  python tests/flea_sell/test_inventory_region_stress.py [rounds]
"""
import random
import sys
