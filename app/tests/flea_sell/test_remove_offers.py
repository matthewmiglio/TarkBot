...
Exercises interact/sell.py's stale_offer_rows and remove_stale_offers, plus the is_on_browse_page
and return_to_browse it leans on. The sweep switches to the my-offers tab, walks the offer rows
bottom upwards clicking every remove button (lowest first, so a cancelled offer cannot shift a
point still to be clicked), settles, and returns to the browse tab. Ending back on browse is the
half the next pass depends on, and it is checked here through the add offer button, which only
exists on the browse tab. Writes remove_offers_before.png, _rows.png (click points numbered in
click order) and _after.png to tests/flea_sell/output/.

This one cancels real offers, so it counts down first and Ctrl+C during the countdown costs
nothing. Needs the live game, flea already open on any tab. find.VERBOSE is turned on so a
failed sweep names which crop stopped matching. Exits non-zero if it never found the my-offers
tab, or did not end up back on browse.

Run:  python tests/flea_sell/test_remove_offers.py
      python tests/flea_sell/test_remove_offers.py --settle 5    (skip most of the 2 minute wait)
      python tests/flea_sell/test_remove_offers.py --quiet       (drop the per-reference-image lines)
"""
import sys
import time
