...
Exercises interact/sell.py's find_sell_pixels and select_item_from_inventory: pick a live pixel
at random, right-click it, and look for the right-click menu's filter_by_item entry to click,
retrying up to SELECT_ATTEMPTS times when a pixel turns out to be unclickable. This is the step
that turns a stash slot into a comparable-offer search. Writes tests/flea_sell/output/select_item.png
with the chosen point ringed.

This one right-clicks and clicks for real (unless --dry, which only shows the pixel it would
pick). Needs the live game with the inventory open and Tarkov the visible window. Exits non-zero
if no attempt landed.

Run:  python tests/flea_sell/test_select_item.py        (countdown, then really right-clicks)
      python tests/flea_sell/test_select_item.py --dry  (show the pixel it would pick, no clicking)
"""
import random
import sys
import time
