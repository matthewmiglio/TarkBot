...
App layer under test: interact/sell.py, sell.open_filters and sell.apply_flea_filters (both the
plain and reset=True paths). Verifies open_filters returns False and clicks only the gear when
flea_filters_window_title never appears, that apply_flea_filters refuses to go on clicking
controls inside a window that never opened, and that a window seen for the wait but gone by the
drag is treated as a failure rather than dragged blind.

No-game stub test: find and pyautogui are both stubbed, so this is about what open_filters does
with the answers, not about matching pixels. No live game or screen.

Run:  python tests/flea_filters/test_filter_window_guard.py
