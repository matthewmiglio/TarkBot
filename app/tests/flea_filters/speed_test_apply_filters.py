...
App layer under test: interact/sell.py, sell.apply_flea_filters, timed rather than checked for
correctness. It divides every pacing sleep the filter pass reads (SCALED below) by a speed that
climbs each round and reports the fastest speed at which all three production shapes of the
call (flea sell, snipe reset, craft-input) still set the filters and have them stick, verified
by reopening the window. Three failing speeds in a row ends it.

Needs the live game: it drives the real mouse the whole time. On the hideout or stash, NOT on
the flea (the loop opens and closes it itself). Do not touch the machine while it runs.
...
Run: python tests/flea_filters/speed_test_apply_filters.py
     python tests/flea_filters/speed_test_apply_filters.py --max 5.0 --step 1.0
"""
import argparse
import os
import sys
