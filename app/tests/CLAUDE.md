...
- `test_scav_case_rolls.py` — craft.scav_case_scroll_positions/find_scav_case_row/lit_get_items + craft_bot.tend_scav_case — both scav case rolls are started and every finished roll is collected whatever the list is scrolled to: a visible roll costs no scrolling, one below the fold is wheeled down to, one above it is found on the sweep back up, a roll that is genuinely absent is skipped rather than raising Blind into a game restart, the panel is reopened exactly once between the start and collect phases, and a lit GET ITEMS on a row with no input icon drawn (which is what a running or finished roll looks like, so the anchored read could never name it and its loot was never taken) is still collected, while a greyed one is left alone — no-game
...
- `test_panel_leave_settle.py` — craft_bot.step (the producing branch, PANEL_LEAVE_SETTLE) + craft.close_open_station_panel (PANEL_CLOSE_ATTEMPTS, PANEL_CLOSE_ESC_SETTLE) — leaving a station waits before navigation clicks that station's close X, a stubborn panel gets five clicks rather than three, and one esc is tried as a last resort before the run ends: the producing branch pauses a real (non-zero) settle before handing over to the swap, a panel that only gives way on the fourth click now closes instead of ending the run, one that never closes still raises after exactly PANEL_CLOSE_ATTEMPTS, esc rescues a panel no click would and is pressed exactly once and only after every click is spent, a panel refusing esc too still raises with 'esc' named in the message, and — the load-bearing one, since a fallback that fires on the ordinary path is not a fallback — a panel that closes on the first click never presses esc at all. From the 2026-09-17 overnight soak, where the workbench panel swallowed its first close click 35 times in 57 and ran out of re-clicks 9 times while six other stations closed first time across ~400 attempts, and the 2026-09-18 one that measured the settle running at 4.19s and the failure surviving it anyway — no-game
- `test_moonshine_no_filters.py` — craft.buy_craft_input_item (`filters`) + craft_bot._start_scav_case_moonshine — the scav case's moonshine bottle is bought off the board as it stands, with no trip through the filter window: a bottle is always 100% condition and only ever sold by players, so every control in that window would be set to what the board already shows, and skipping it saves ~15s on the most frequent buy in the cycle (53 trips to that board in the 3h soak of 2026-09-18). Checks that `filters=False` skips the window and still buys the cheap offer, that the default still opens it exactly once (every other input's condition and source genuinely vary), and — the load-bearing one, since a flag nothing sets is dead code that reads like a feature — that the scav case path really passes it — no-game
- `test_collect_retry.py` — craft_bot.collect_craft (GET_ITEMS_ATTEMPTS) + craft.get_items_cleared — a GET ITEMS click the game never registered is clicked again up to 3 times, and the craft's profit is booked only once the button has actually cleared: a swallowed first click is retried and re-jittered, a button that never clears is bounded and returns None without raising and without booking profit for loot that never arrived, and get_items_cleared ignores both a neighbouring row's lit button (which the padded band overlaps) and a greyed one — no-game
