...
- `test_panel_leave_settle.py` — craft_bot.step (the producing branch, PANEL_LEAVE_SETTLE) + craft.close_open_station_panel (PANEL_CLOSE_ATTEMPTS) — leaving a station waits before navigation clicks that station's close X, and a stubborn panel gets five clicks rather than three: the producing branch pauses a real (non-zero) settle before handing over to the swap, a panel that only gives way on the fourth click now closes instead of ending the run, and one that never closes still raises after exactly PANEL_CLOSE_ATTEMPTS. From the 2026-09-17 overnight soak, where the workbench panel swallowed its first close click 35 times in 57 and ran out of re-clicks 9 times while six other stations closed first time across ~400 attempts — no-game
