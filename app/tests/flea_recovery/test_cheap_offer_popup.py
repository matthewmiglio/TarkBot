...
App layer: sell_bot.FleaSeller.sell_one's failure/back-out paths, over interact/sell.py's
cheap_offer_popup (the below-market "are you sure" confirmation) and get_price returning None
(the unreadable-price back-out). Verifies the stats a refused or unpriced pass records and the
escape count it uses to unwind every window it opened.

Run:  python tests/flea_recovery/test_cheap_offer_popup.py

No game needed, nothing is clicked (a pure stub/logic test: every screen-touching sell.* call is
stubbed). sell_one is driven with every screen-touching call stubbed,
