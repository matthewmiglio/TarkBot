...
App layer: interact/sell.py's dismiss_error_popup, which clears Tarkov's plain "Error / 0 / OK"
dialog by clicking OK at ERROR_POPUP_OK_FRACTION down the matched dialog box. Verifies that
arithmetic (box middle in x, OK's glyph rows in y) for every crop in error_0_popup/, at both a
positive and a negative screen origin, and that no dialog means no click.

Run:  python tests/flea_recovery/test_error_popup.py

No game needed, nothing is really clicked (a pure stub test: find and pyautogui are stubbed), so
this is about the
