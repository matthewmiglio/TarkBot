...
App layer: interact/sell.py's close_leftover_windows (the single-sweep primitive Start's
recover loop is built from). This pins down which leftover windows it decides to back out of and
which ordinary screens (the plain inventory) it must leave alone.

Run:  python tests/flea_recovery/test_recover_on_start.py

No game needed, nothing is clicked (a pure stub/logic test: find and pyautogui are faked).
sell.close_leftover_windows is driven with find() faked, so
