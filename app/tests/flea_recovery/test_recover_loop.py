...
App layer: sell_bot.FleaSeller._recover, the recover/unwind Start runs before it looks for the
flea, built on interact/sell.py's close_leftover_windows and open_flea. This exercises the
recovery loop's control flow; the crop-matching half is test_recover_targets.py's job.

Run:  python tests/flea_recovery/test_recover_loop.py

No game needed, nothing is clicked (a pure stub/logic test: find, sell and pyautogui are all
faked). FleaSeller._recover is driven against a fake screen: a list of
