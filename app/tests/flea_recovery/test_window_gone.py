...
App layer: sell_bot.FleaSeller.open_offer_creation, and specifically the window.handle check it
makes just before the add offer click. Verifies a game closed mid-run raises WindowError and
clicks nothing (rather than clicking a stale region over whatever is now behind it), while an
open game still runs the click through.

Run:  python tests/flea_recovery/test_window_gone.py

No game needed, nothing is clicked (a pure stub/logic test: window.handle and every
screen-touching sell.* call are faked). open_offer_creation is driven with every screen-touching
