...
App layer: sell_bot.FleaSeller._recover and the crops interact/sell.py's recovery reads for
(OFFER_TARGET, SCAV_WINDOW_TARGET, FILTERS_WINDOW_TARGET, FLEA_ICON_TARGET) matched against the
live game through interact/find.find. The half test_recover_loop.py cannot cover: whether the
shipping crops still match the real UI.

Run:  python tests/flea_recovery/test_recover_targets.py          report what is on screen, press nothing
      python tests/flea_recovery/test_recover_targets.py --run    run the real recovery, clicks and all

NEEDS THE LIVE GAME (builds a real FleaSeller, which raises WindowError if Tarkov is not up).
Bare it only reports what matches and presses nothing; --run clicks for real. Needs the game up,
with whatever you want checked already on screen. The default reads only, so
...
REFS = Path(__file__).resolve().parents[2] / 'interact' / 'reference_images'
