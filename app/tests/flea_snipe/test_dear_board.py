...
App layer under test: craft.buy_craft_input_item, which reads the top offer's price through
interact/snipe.py's read_price (the flea BUYER's read) and decides buy / wait / refresh / give
up against a ceiling. Here it is the wait-and-refresh budget and the pre-filter shortcut being
pinned, not the pixel read.

Run:  python tests/flea_snipe/test_dear_board.py
