...
App layer under test: interact/snipe.py's ruble_region and RUBLE_ROI_FRACTIONS, the balance box
the flea BUYER photographs either side of a click to confirm the roubles actually left the
stash (snipe.buy / purchase_landed). This is that box's tuning loop.

Run:  python tests/flea_snipe/test_ruble_region.py              against the live game
      python tests/flea_snipe/test_ruble_region.py frame.png    against a saved full-window screenshot
...
OUT = Path(__file__).resolve().parents[1] / 'output' / 'ruble_region'
ZOOM = 4
