...
Run:  python tests/test_start_ready.py                 (grab the live window)
      python tests/test_start_ready.py <frame.png ...>  (saved frames, no game needed)

Reads brightness straight off the image it is annotating and compares to
craft.START_READY_BRIGHTNESS, so the box colour is the real decision, not a re-implementation of
it. Writes tests/output/start_ready/<name>.png and prints the brightest channel per button.
"""
import sys
...
OUT = Path(__file__).parent / 'output' / 'start_ready'
GREEN = (0, 220, 0)
RED = (220, 0, 0)
