...
App layer: interact/sell.py's click spread (sell.jitter) and the three wired call sites that
apply it (sell.click_add_offer, sell.enter_price, sell.click_place_offer), bounded by the
smallest reference crop under interact/reference_images/ for each target.

Run:  python tests/detection/test_click_jitter.py

NO GAME needed, nothing is clicked: find and pyautogui are faked so the offset reaches a
recorder instead of the mouse. jitter() is called directly, and the seven wired-up call
