"""Show where interact/craft.py's craft-row finder puts the slickers band on a crafting screen.

App layer under test: interact/craft.py's screen READ that locates a craft's row, the
find_slickers_craft / find_craft / output_box family (not navigation, not buying). It matches the
OUTPUT slickers icon (the one with a timer clock, TIMER_TARGET, to its left on the same row),
pads it into the full-width band that find() is later handed as the region for that craft's
ingredients, marks and START, and this replays that geometry (ROW_TOL, SLICKERS_BAND_PAD,
SLICKERS_BAND_TOP_DROP) so the band drawn is the module's, not a copy.

What it verifies: that the output row is found and the band drawn around it is sane, checked by
eye from the picture rather than by an assertion.
...
Run:  python tests/hideout_craft_reads/test_find_slickers_craft_region.py                 (grab the live window)
      python tests/hideout_craft_reads/test_find_slickers_craft_region.py <frame.png ...>  (saved frames, no game)
...
OUT = Path(__file__).resolve().parents[1] / 'output' / 'slickers_region'
