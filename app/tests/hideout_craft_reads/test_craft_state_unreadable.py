...
App layer under test: interact/craft.py's craft.get_craft_state, the READ that classifies a
craft's row (not started / done / ready / producing) and now raises craft.Blind when the output is
not on screen at all, instead of guessing 'producing'. Also that the raise travels out through
craft_bot.HideoutCraft.step and start() rather than being swallowed into another lap. Read layer
plus the runner's reaction, not navigation and not buying.

What it verifies: a missing output raises Blind (which is not a LookupError, so step()'s swap
catch cannot eat it); with an output present the row still reads 'ready' (START on it) and
'producing' (neither button); and the Blind from the state read escapes start() on the first lap.
...
Run:  python tests/hideout_craft_reads/test_craft_state_unreadable.py
