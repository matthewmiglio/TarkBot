"""Print interact/craft.py's craft-state read for a crafting screen.

App layer under test: interact/craft.py's get_slickers_craft_state / get_craft_state, the READ
that classifies a craft's own row as one of not started / done / ready / producing (GET ITEMS
greyed vs lit, a START button present, or none of those). On a saved frame the same logic is
replayed against the image so the reported state is the module's, with the four targets
(TIMER_TARGET, SLICKERS_TARGET, GET_ITEMS_TARGET, START_TARGET) and the GET ITEMS brightness
threshold (GET_ITEMS_HIGHLIGHT_BRIGHTNESS) coming straight from craft. Not navigation, not buying.

What it verifies: that the four states are reported correctly off the row, by printing the state
...
Run:  python tests/hideout_craft_reads/test_get_slickers_craft_state.py                 (grab the live window)
      python tests/hideout_craft_reads/test_get_slickers_craft_state.py <frame.png ...>  (saved frames, no game)
"""
import sys
