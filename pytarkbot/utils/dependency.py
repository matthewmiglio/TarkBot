...
from os.path import abspath, dirname, exists, expandvars, join, normpath, pardir
...
top_level = join(expandvars(r"%appdata%"), "py-TarkBot")
    ...
    if not exists(join(output_dir, file_name)):
        ...
        try:
            with DownloadProgressBar(
                ...
                urlretrieve(
                    url, filename=join(output_dir, file_name), reporthook=t.update_to
                )  # nosec
...
def get_tsrct_link() -> list[str] | None:
    ...
    latest_ver = ver_df[ver_df["Description"].str.contains("latest", na=False)][
        "Name"
    ].values[0]
...
def install_tesseract() -> None:
    ...
    dependency_dict = {"tesseract": get_tsrct_link()}
