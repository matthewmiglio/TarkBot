...
def get_color_list_of_current_price(image):
    ...
    while x_coord<115:
        ...
        current_color=None
        if (pixel_is_equal(current_pix,color_tan,tol=5))or(pixel_is_equal(current_pix,color_tan2,tol=5)):
            current_color="tan"
...
def check_price_string(price_string):
    ...
    for element in price_string:
        ...
        index=index+1
        if not((element =="1")or(element =="2")or(element =="3")or(element =="4")or(element =="5")or(element =="6")or(element =="7")or(element =="8")or(element =="9")or(element =="0")):
            return None
...
def find_coords_of_item_to_flee():
    ...
    #return one of the coords at random that aren't the background's black color.
...
def check_if_item_to_flee_is_trader_item(logger):
    ...
    if truth==True: logger.log("Item is trader item")
    return truth
...
def check_if_on_flee_page(logger):
    ...
    truth=check_for_location(locations)
    if truth==True: logger.log("On flee page.")
    return truth
...
def wait_till_can_add_another_offer(logger):
    ...
    while waiting:
        ...
        if loops>120:
            logger.log("Waited more than 120sec for any of the offers to sell. Passing to remove offers state.")
...
def remove_offers(logger):
    ...
    while loops<10:
        ...
        if remove_button_coords is not None:
            ...
            pyautogui.press('y')
            time.sleep(0.33)
        #click item filters on left to assure not systematically missing an offer.
...
def look_for_remove_offer_button():
    ...
    if (pix_list is None)or(pix_list==[]): return None
    return pix_list[0]
